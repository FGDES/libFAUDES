/** @file luafaudes.cpp Simple Lua stand-alone interpreter for lua/faudes
   ...
   Changes to the original code are signed "//TM". They include
...
//TM:: use my include file (incl. lpb_include.h and, hence, lua.h & friends)
#include "libfaudes.h"

//TM: import all libFAUDES
using namespace faudes;

//TM: insist in lua 5.4.x
#if LUA_VERSION_NUM < 504 
#error "This verion of luafaudes is meant for Lua 5.4.x"
#endif

//TM: dislike isatty on MSYS
...
//TM: changed name form 'lua' to 'luafaudes'
...
//TM: libFAUDES specific version/greeting.
...
#define has_e		8	/* -e */
#define has_E		16	/* -E */
#define has_x		32	/* //TM: -x */
#define has_d		64	/* //TM: -d */
    ...
    *first = i;
    ...
    switch (argv[i][1]) {  /* else check option */
      ...
      case 'd':     /* //TM: d-option */
        ...
        args |= has_d;
        break;
      case 'x':	/* //TM: -x: needs an argument and sets the flag */
        if (argv[i][2] == '\0') {  /* no concatenated argument? */
          i++;  /* try next 'argv' */
          if (argv[i] == NULL || argv[i][0] == '-')
            return has_error;  /* no next argument or it is another option */
  ...
  for (i = 1; i < n; i++) {
    ...
    switch (option) {
      ...
      case 'W':
        ...
        break;
      case 'x': {  /* //TM: load faudes extension */
...
/*
** //TM: here we to be advanced readline support patch by Mike Pall. However,
...
static void lua_initreadline(lua_State *L) {
  ...
  rl_completer_word_break_characters = (char *) 
    "\t\r\n !\"#$%&'()*+,-/;<=>?@[\\]^`{|}~"; //TM: pragmatic avoidance of compiler warning

  rl_completer_quote_characters = "\"'";
  rl_completion_append_character = '\0';
  rl_attempted_completion_function = faudes_complete_L; //TM: use libFAUDES variant
  rl_initialize();
...
static int pmain (lua_State *L) {
  ...
  luaopen_faudes_allplugins(L); /* //TM: load all faudes bindings */
  ...
  if (!runargs(L, argv, optlim))  /* execute arguments -e, -l  //TM and -x */
    return 0;  /* something failed */
  if (!(args & has_x))
    faudes_loaddefext(L, argv[0]);  /* //TM: load default extension */
