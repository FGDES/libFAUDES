 ...
 *
 * Convert a generator accepting the language Lm into a formal automaton recognizing Lm
 * with a dump state representing L-Sigma^*. In this function, Sigma is given by the
 * alphabet of rGen; see also Automaton(rGen,rAlphabet).
 *
 * Note: An automaton is a deterministic transition structure according to the formal
 *       definition; see also "Determinism" below.
 *
 * Note: pre libFAIDES v2.33 we trimmed the automaton since non-coaccessible states
 *       wont contribute to Lm. This was dropped to be tolerant to alternative acceptance
 *       conditions. Preprocess with Trim explicitly to obtain pre v2.33 semantics.
 *
 * Method:
 * Unaccessible states are erased, as they do not contribute to the languages generated
 * or accepted by rGen. A dump state representing "L-Sigma*" is created. Then, the
 * transition relation is completed such that it is fully defined for each state and
 * each event. Formerly undefined transitions lead to the dump state.
 *
 * Determinism:
 ...
 * does not represent "Sigma*-PrefixClosure(Lm)" as it should; see also
 * example ExAutomaton_basic(). If FAUDES_CHECKED is defined a warning on non-deterministic
 * input is issued.
 * 
 ...
 * @ingroup GeneratorFunctions
 */
extern FAUDES_API Idx Automaton(Generator& rGen);

/**
 * Convert generator to automaton.
 *
 * This is an API wrapper for Automaton(rGen,rGen).
 *
 * @param rGen
 *    input generator
 * @param rRes
 ...
 *   index of dumpstate (0 for none)
 */
extern FAUDES_API Idx Automaton(const Generator& rGen, Generator& rRes);

/**
 * Convert generator to automaton wrt specified alphabet.
 *
 * Convert a generator accepting the language Lm into a formal automaton recognizing Lm
 * with a dump state representing L-Sigma^*. In this function, Sigma is specified explixitly.
 *
 * @param rGen
 *   generator that is converted to automaton
 * @param rAlphabet
 *   the dump state of the resulting automaton represents the
 *   language L_dump=rAlphabet*-L
 * @return
 *   index of dumpstate (0 for none)
 *
 * @ingroup GeneratorFunctions
 */
extern FAUDES_API Idx Automaton(Generator& rGen, const EventSet& rAlphabet);
